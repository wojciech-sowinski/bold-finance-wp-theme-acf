    ...
    <div class="col-12 col-lg-6 bg-primary text-light d-flex flex-column justify-content-start align-items-start">
      <div class="col-12 p-1 px-md-5 py-md-4 ">
        ...
        <div>
          ...
          <div
            class="py-2 d-flex  flex-column flex-md-row  justify-content-center justify-content-lg-start align-items-center gap-5">
            <?php $btnText = get_field('target_cta_header_btn_text'); ?>
            <?php $target_cta_header_btn_url = get_field('target_cta_header_btn_url'); ?>
            <?php if ($target_cta_header_btn_url): ?>
              <?php $post = $target_cta_header_btn_url; ?>
              <?php setup_postdata($post); ?>
              <a class="btn btn-secondary" href="<?php the_permalink(); ?>"><?php echo $btnText ?></a>
              <?php wp_reset_postdata(); ?>
            <?php endif; ?>
            <?php if (have_rows('target_add_attachments')): ?>
              <?php while (have_rows('target_add_attachments')):
                the_row(); ?>
                <?php $target_add_attachments_link = get_sub_field('target_add_attachments_link'); ?>
                <?php if ($target_add_attachments_link): ?>
                  <?php $post = $target_add_attachments_link; ?>
                  <?php setup_postdata($post); ?>
                  <a class="text-white text-decoration-underline" title="<?php the_sub_field('target_attachment'); ?>"
                    href="<?php the_permalink(); ?>"><?php the_sub_field('target_attachment'); ?></a>
                  <?php wp_reset_postdata(); ?>
                <?php endif; ?>
              <?php endwhile; ?>
            <?php endif; ?>
...
<?php if (have_rows('product_tab')): ?>
  <section class="container pt-2 pb-2 ">
    <div class="container">
      <div class="row">
        <ul class="nav nav-tabs d-flex justify-content-center mt-3 mb-5 align-items-center" id="nav-tabs-dot"
          role="tablist">
